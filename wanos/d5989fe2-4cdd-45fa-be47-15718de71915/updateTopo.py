...
numTypes={'DHPMA':[48, 47, 84, 115, 26], 'HEMA':[40, 39, 70, 91, 22], 'EGDMA':[18, 17, 28, 28, 9], 'TRP':[27, 28, 46, 69, 15], 'DEAE':[20, 19, 36, 45, 12], 'SO3':[5, 3, 3, 0, 0]}
...
def candDEAE(a, ldall, aoffset):
    Aidx1=findmolecule(a, 7+aoffset, ldall) #R H
    Aidx2=findmolecule(a, 1+aoffset, ldall) #R C
    Aidx3=findmolecule(a, 3+aoffset, ldall) #R N
    #spacer
    ALeftL=findmolecule(a, ldall.natomtypes-1, ldall)
    ARightL=findmolecule(a, ldall.natomtypes, ldall)
    return Aidx1, Aidx2, Aidx3, ALeftL, ARightL
    
def candSO3(a, ldall, aoffset):
...
for bid, bond in enumerate(ldref.bonds):
    ...
    if typA=="Monomer":
        ...
        if typLigand=="TRP":
            ...
            candA=candDEAE(a, ldall, aoffset=numTypes[typMonomer][0]+numTypes[typXLinker][0])
        elif typLigand=="SO3":
            candA=candSO3(a, ldall, aoffset=numTypes[typMonomer][0]+numTypes[typXLinker][0])
    ...
    if typB=="Monomer":
        ...
        if typLigand=="TRP":
            ...
            candB=candDEAE(b, ldall, aoffset=numTypes[typMonomer][0]+numTypes[typXLinker][0])
        elif typLigand=="SO3":
            candB=candSO3(b, ldall, aoffset=numTypes[typMonomer][0]+numTypes[typXLinker][0])
...
with open("COrecorder.dat", 'w') as f:
    for idx in range(len(carMarker)):
        car=carMarker[idx]
        carreID=car-sum(i<car for i in delatoms)+1
        oxy=oxyMarker[idx]
        oxyreID=oxy-sum(i<oxy for i in delatoms)+1
        #print("{} {}".format(carreID, oxyreID))
        f.write("{} {}\n".format(carreID, oxyreID))
