...
class LammpsData:
    ...
    def analyseLine(self, ln, line):
        ...
        if self.currentsection == "Masses":
            if len(elements) == 2:
                ...
                self.masses[int(elements[0])]=float(elements[1])
                return
    ...
    def validateData(self):
        ...
        self.natomtypes=np.max([k for i,(k,atom) in enumerate(self.masses.items())])#np.max([atom[1] for i,(k,atom) in enumerate(self.atoms.items())])
    ...
    def writeFile(self, outfile, reorder=False, autoID=True):
        ...
        with open(outfile, 'w') as of:
            ...
            for i, mw in enumerate(self.masses):
                #of.write("{:{mw}d} {:7.4f}\n".format(i+1,m,mw=mw))
                of.write("{:d} {:7.4f}\n".format(mw,self.masses[mw]))
